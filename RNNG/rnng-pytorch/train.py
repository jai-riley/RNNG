...
def main(args):
  ...
  do_valid(model, optimizer, scheduler, train_data, val_data, tb, epoch, -1, val_losses, val_word_ppls, args)
  while epoch <= args.num_epochs and not is_early_stop():
    ...
    for batch in train_data.batches():
      ...
      if args.valid_every > 0 and global_batch_i % args.valid_every == 0:
        do_valid(model, optimizer, scheduler, train_data, val_data, tb, epoch, continuous_epoch, val_losses,
                 val_word_ppls, args)

    output_learn_log()
    epoch += 1
    if args.valid_every <= 0:
      do_valid(model, optimizer, scheduler, train_data, val_data, tb, epoch, epoch, val_losses, val_word_ppls, args)
...
def do_valid(model, optimizer, scheduler, train_data, val_data, tb, epoch, step, val_losses, val_word_ppls, args):
  best_val_loss = float('inf') if len(val_losses) == 0 else min(val_losses)
  logger.info('--------------------------------')
  logger.info('Checking training perplexity...')
  train_loss, train_ppl, train_action_ppl, train_word_ppl = eval_action_ppl(train_data, model)
  tb.write({'Train ppl': train_ppl, 'Train action ppl': train_action_ppl, 'Train word ppl': train_word_ppl}, step)
  tb.write({'Train loss': train_loss}, use_time=True)
  logger.info('--------------------------------')
  logger.info('Checking validation perplexity...')
  val_loss, val_ppl, val_action_ppl, val_word_ppl = eval_action_ppl(val_data, model)
  val_word_ppls.append(val_word_ppl)
...
def eval_action_ppl(data, model):
  ...
  ppl = np.exp((-total_a_ll - total_w_ll) / (num_actions + num_words))
  loss = -(total_a_ll + total_w_ll) / num_batches
  action_ppl = np.exp(-total_a_ll / num_actions)
  word_ppl = np.exp(-total_w_ll / num_words)
